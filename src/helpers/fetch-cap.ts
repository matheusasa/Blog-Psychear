// helpers/fetch-cap.ts

import config from "@/config";

const fetchCapituloDetalhes = async (capituloId: number) => {
    const reqOptions: RequestInit = {
      headers: {
        Authorization: `Bearer `,
      },
    };
  
    try {
      const request = await fetch(
        `${config.api}/api/capitulos/${capituloId}?populate[aulas][populate]=*`, // Ajuste a URL se necessário
        reqOptions
      );
  
      if (!request.ok) {
        throw new Error(`Network response was not ok: ${request.statusText}`);
      }
  
      const response = await request.json();
      return response;
    } catch (error) {
      console.error("Fetch error:", error);
      return { data: { attributes: { aulas: { data: [] } } } };
    }
  };
  
  export { fetchCapituloDetalhes };
  